...
public class EnemyCollision {
    public static boolean withExplosion(int x, int y, int BombX, int BombY) {
        ...
        int Ox = (x + (Sprite.SCALED_SIZE)/2) - (BombX + 2 + (2 * DirectionalExplosion.radius * Sprite.SCALED_SIZE + Sprite.SCALED_SIZE - 2)/2);
        int Oy = (y + Sprite.SCALED_SIZE/2) - 2 - (BombY + 2 +  (Sprite.SCALED_SIZE - 2)/2);
        if (Ox < 0) Ox = -Ox;
        if (Oy < 0) Oy = -Oy;
        ...
        Ox = (x + Sprite.SCALED_SIZE/2) - (BombX + 2 + (Sprite.SCALED_SIZE - 2)/2);
        Oy = (y  + Sprite.SCALED_SIZE/2) - (BombY + 2 +  (2 * DirectionalExplosion.radius * Sprite.SCALED_SIZE + Sprite.SCALED_SIZE - 2)/2);
        if (Ox < 0) Ox = -Ox;
        if (Oy < 0) Oy = -Oy;
